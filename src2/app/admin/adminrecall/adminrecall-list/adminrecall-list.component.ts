...
export class AdminrecallListComponent implements OnInit {
  ...
  recallList : Recall[]
  constructor(public db: AngularFireDatabase,public recallService: RecallService) { }
  ...
  onEdit(recall : Recall){
    this.recallService.selectedRecall = recall
  }

  onDelete(key: string){
    this.recallService.deleteRecall(key)
  }
